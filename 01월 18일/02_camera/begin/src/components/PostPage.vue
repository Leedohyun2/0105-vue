<template>
    <v-container>
        ...
        <!-- 두 번째 행에는 기본 제목을 표시하고 수정할 수 있도록함 -->
        <v-row mt-5>
            <v-col offset="2" cols="8">
                <v-text-field name="title" label="사진제목" v-model="sTitle" autofocus>
                </v-text-field>
            </v-col>
        </v-row>
        ...
        <v-row>
            <v-col offset="2" cols="8">
                <!-- 3줄로 편집 제한 -->
                <v-text-field name="info" label="사진 설명" v-model="sInfo" multi-line rows="3">
                </v-text-field>
            </v-col>
        </v-row>
...
export default {
    ...
    mounted(){
        // JSON파일에서 사진 정보를 랜덤으로 읽어와서 사진과 포스트글 준비
        let nIndex = Math.floor(Math.random() * 12)
        const itemPic = this.aPictures[index]
